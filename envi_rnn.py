...
plt.ylabel("CC")
plt.xlabel("Sample")
plt.savefig("plots/cc_test_"+desc_str+".png")
...
plt.ylabel("CC")
plt.xlabel("Sample")
plt.savefig("plots/cc_train_"+desc_str+".png")
"""
...
#for p in range(PAST_SAMPLES):
#    predictions, hidden = network(X_test_tensor)
predictions, hidden = network(X_test_tensor)
...
#for p in range(PAST_SAMPLES):
#    predictions, hidden = network(X_train_tensor)
predictions, hidden = network(X_train_tensor)
final_predictions = predictions.view(-1,PAST_SAMPLES,WF_SIZE)[:,-1,:]
